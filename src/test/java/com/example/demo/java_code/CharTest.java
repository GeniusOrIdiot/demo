...
public class CharTest {
    ...
    public static String changeUtf8ToUnicode(byte[] aByte) {
        ...
        StringBuilder sUnicodeStringBuffer = new StringBuilder();
        ...
        for (int i = 0; i < sLength; i++) { //循环每一个字节
            if (i + 2 < sLength) {
                /*
                  ...
                  假如条件都满足，则表示此断字节进行了utf-8编码，则将对其进行解码操作（即转化为unicode编码）
                 */
                if ((aByte[i] & 0xF0) == 0xE0 && (aByte[i + 1] & 0xC0) == 0x80 && (aByte[i + 2] & 0xC0) == 0x80) {
                    /*
